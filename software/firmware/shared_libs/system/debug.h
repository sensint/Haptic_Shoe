#ifndef __SENSINT_DEBUG_H__
#define __SENSINT_DEBUG_H__
 ...
 */
static void printToDisplay(const String& message) {
  // placeholder
}
...
#else
const auto kDebugLevel = (SENSINT_DEBUG == 1) ? DebugLevel::basic : DebugLevel::verbose;
#endif  // SENSINT_DEBUG
 ...
 */
static void Log(const String& message, const DebugLevel level = DebugLevel::basic) {
 ...
 * @param level the level of the message
 */
static void Log(const String& file, const String& func, const int line, const String& message,
                const DebugLevel level = DebugLevel::basic) {
  if (static_cast<uint8_t>(level) <= static_cast<uint8_t>(kDebugLevel)) {
